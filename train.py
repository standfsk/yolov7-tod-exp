        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            with amp.autocast(enabled=cuda):
                ...
                if 'loss_ota' not in hyp or hyp['loss_ota'] == 1:
                    loss, loss_items = compute_loss_ota(pred, targets.to(device), imgs, opt.iou_type,
                                                        nwd_loss=opt.nwd)  # loss scaled by batch_size
                else:
                    loss, loss_items = compute_loss(pred, targets.to(device), opt.iou_type,
                                                    nwd_loss=opt.nwd)  # loss scaled by batch_size
                if rank != -1:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--iou_type', default='ciou', help='e.g ciou, wiou, focal_eiou')
    parser.add_argument('--nwd', action='store_false', help='NWD Loss')
