...
class DeformConv(nn.Module):
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, act=True):  # ch_in, ch_out, kernel, stride, padding, groups
        ...
        self.offsets = nn.Conv2d(c1, g*2*k*k, k, s, autopad(k, p), groups=g, bias=False)
        self.deform_conv = DeformConv2d(c1, c2, k, s, autopad(k, p), groups=g, bias=False)
        self.bn = nn.BatchNorm2d(c2)
        self.act = nn.SiLU() if act is True else (act if isinstance(act, nn.Module) else nn.Identity())

    def forward(self, x):
        offsets = self.offsets(x)
        return self.act(self.bn(self.deform_conv(x, offsets)))

    def fuseforward(self, x):
        offsets = self.offsets(x)
        return self.act(self.deform_conv(x, offsets))
