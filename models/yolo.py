...
class IDetect(nn.Module):
    ...
    def fuseforward(self, x):
        ...
        for i in range(self.nl):
            ...
            feature_visualization(x[i], module_type="IDetect", stage=i, save_dir="features")
...
class IDetect_Decoupled(nn.Module):
    ...
    def fuseforward(self, x):
        ...
        for i in range(self.nl):
            ...
            feature_visualization(x[i], module_type="IDetect_Decoupled", stage=i, save_dir="features")
...
class Model(nn.Module):
    ...
    def forward_once(self, x, profile=False):
        ...
        for m in self.model:
            ...
            if self.traced:
                if isinstance(m, Detect) or isinstance(m, IDetect) or isinstance(m, IAuxDetect) or isinstance(m, IKeypoint) or isinstance(m, IDetect_Decoupled):
                    break
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in [nn.Conv2d, Conv, RobustConv, RobustConv2, DWConv, GhostConv, RepConv, RepConv_OREPA, DownC, 
                 ...
                 CBAM, Involution, SENet, CoordAttn, CFEM, HGStem, LHGBlock, LGSCSP, DeformConv, MEAP, EAC3_n, EAC3_nF, nn.ConvTranspose2d, MDC, DC]:
            c1, c2 = ch[f], args[0]
...
def feature_visualization(x, module_type, stage, n=32, save_dir=Path('runs/detect/exp')):
    ...
    """
    run_vis = False
    if run_vis: #'Detect' not in module_type:
        batch, channels, height, width = x.shape  # batch, channels, height, width
        if height > 1 and width > 1:
            f = os.path.join(save_dir, f"stage{stage}_{module_type.split('.')[-1]}_features.png")  # filename
            ...
            ax[0].imshow(gray_scale.detach().numpy().squeeze())
            ax[0].axis('off')
