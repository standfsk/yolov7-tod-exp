...
def wasserstein_loss(pred, target, eps=1e-7, constant=12.8, weight=2):
    ...
    #
    # center1 = (pred[..., :, None, :2] + pred[..., :, None, 2:]) / 2
    # center2 = (target[..., None, :, :2] + target[..., None, :, 2:]) / 2
    # whs = center1[..., :2] - center2[..., :2]
    #
    # center_distance = whs[..., 0] * whs[..., 0] + whs[..., 1] * whs[..., 1] + eps  #
    #
    # w1 = pred[..., :, None, 2] - pred[..., :, None, 0] + eps
    # h1 = pred[..., :, None, 3] - pred[..., :, None, 1] + eps
    # w2 = target[..., None, :, 2] - target[..., None, :, 0] + eps
    # h2 = target[..., None, :, 3] - target[..., None, :, 1] + eps
    #
...
class ComputeLoss:
    ...
    def __call__(self, p, targets, iou_type, nwd_loss=True):  # predictions, targets, model
        device = targets.device
        lcls, lbox, lobj = torch.zeros(1, device=device), torch.zeros(1, device=device), torch.zeros(1, device=device)
        tcls, tbox, indices, anchors = self.build_targets(p, targets)  # targets

        iou_ratio = [0.5, 0.7, 0.9, 0.95]
        # Losses
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                elif iou_type == 'focal_eiou':
                    iou = bbox_iou(pbox.T, tbox[i], x1y1x2y2=False, EIoU=True, Focal=True)
                elif iou_type == 'siou':
                    iou = bbox_iou(pbox.T, tbox[i], x1y1x2y2=False, SIoU=True)
                elif iou_type == 'focal_siou':
                    iou = bbox_iou(pbox.T, tbox[i], x1y1x2y2=False, SIoU=True, Focal=True)
                if nwd_loss:
                    nwd = wasserstein_loss(pbox, tbox[i])
                    if isinstance(iou, tuple):
                        if len(iou) == 2:
                            lbox += (1 - iou_ratio[i]) * (1.0 - nwd).mean() + iou_ratio[i] * ((1.0 - iou[0]) * iou[1].detach()).mean()
                            iou = iou[0]
                        else:
                            lbox += (1 - iou_ratio[i]) * (1.0 - nwd).mean() + iou_ratio[i] * (iou[0] * iou[1]).mean()
                            iou = iou[-1]
                    else:
                       lbox += (1 - iou_ratio[i]) * (1.0 - nwd).mean() + iou_ratio[i] * (1.0 - iou).mean()

                    # Objectness
                    tobj[b, a, gj, gi] = (1.0 - self.gr) + self.gr * (iou.detach() * iou_ratio[i] + nwd.detach() * (1 - iou_ratio[i])).clamp(0).type(tobj.dtype)  # iou ratio
                else:
                    if isinstance(iou, tuple):
                        if len(iou) == 2:
                            lbox += ((1.0 - iou[0]) * iou[1].detach()).mean()
                            iou = iou[0]
                        else:
...
class ComputeLossOTA:
    ...
    def __call__(self, p, targets, imgs, iou_type, nwd_loss=True):  # predictions, targets, model   
        device = targets.device
        lcls, lbox, lobj = torch.zeros(1, device=device), torch.zeros(1, device=device), torch.zeros(1, device=device)
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                if iou_type == 'iou':
                    ...
                    iou = bbox_iou(pbox.T, selected_tbox, x1y1x2y2=False, EIoU=True)
                elif iou_type == 'focal_eiou':
